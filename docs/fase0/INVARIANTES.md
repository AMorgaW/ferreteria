...
| INV-11 | `device_id` UUID persistente ≠ hostname | Columna sync vacía; no hay `device_identity.json` | XFAIL `test_contrato_device_identity` |
...
| INV-18 | Registries de sync coherentes | 5 listas; `PULL_ORDER` muerto; `SYNCED_TABLES` incompleto | CAR `test_registries_divergen` |
...
`OFFLINE_INVENTORY_AUTHORITY` y de `device_identity.json`.
No se implementa fencing en Fase 0.
